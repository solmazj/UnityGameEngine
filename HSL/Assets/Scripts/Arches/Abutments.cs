...
//is attached to structures; eg circular or corbel
...
public class Abutments : MonoBehaviour {
	...
	float topHeight, leftPoint;
	GameObject parent;
	GameObject prefab;


	public void SetAbutmentPositions(float pos) {
		//structure (Corbel/BezierCurve) scripts feed the left (negative) point to this script
		leftPoint = pos;
	}
	...
	public void BuildAbutment () {
		...
		if (wallDepth <= brickDepth) {
			...
			//create the left abutment; leftpoint is already negative number so -wallLength points the left end of the wall (aka abutment)
			...
			//the x position is centered around 0. See Final ReadMe for details.
	...
	void CreatePrefabBrick () {
		//creates a prefab with the given dimensions
	...
	void DrawWall (Vector3 position) {
		//draw lanes as long as they can fit
	...
	void DrawOddLane (Vector3 position) {
		...
		//alternayingly draw odd and even rows as long as whole rows can fit in a lane
	...
	void DrawEvenLane (Vector3 position) {
		//same as DrawOddLane, but this starts with DrawEvenRow and then alternates with Odd Row
		topHeight = brickHeight;
		while ((position.y - brickHeight/2) < wallHeight)
		{
			if (2 * (wallHeight - position.y) < brickHeight) {
	...
	void DrawOddRow (Vector3 position) {
		...
		//draw end brick if one is needed
	...
	GameObject DrawBrick(Vector3 position) {
		...
		//if on last row, need to change the height, aka y-scale
	...
	void DrawEndBrick (Vector3 position) {
		//creates a brick for the end of the row, smaller x-scale (+ y-scale if on top row)
		GameObject brick = DrawBrick (position);
		if (topHeight != brickHeight) {
