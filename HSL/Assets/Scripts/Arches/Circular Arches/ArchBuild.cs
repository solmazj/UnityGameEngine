...
//a part of this script is adapted from https://gamedev.stackexchange.com/questions/5373/moving-ships-between-two-planets-along-a-bezier-missing-some-equations-for-acce/5427#5427
public class ArchBuild : MonoBehaviour {

	//parameters input from the Inspector
	public int numberOfBricks;
	public float archThickness, brickDepth, vaultDepth;
	//create the Bezier Curve that would be the skeleton of the arch
	BezierCurve wireArc;
	//in how many steps the info about bezier curve is calculated, most efficient when = 100
	int steps = 100;
	//arc lengths along the arc
	float[] arcLengths;
	//the total arc (aka curve) length, the last entry in arcLengths array 
	float curveLength;  
	//variables calculated for the wedged bricks
	float innerBrickLength, outerBrickLength;
	...
	void Awake () {
		//attaching a CircularTunnel script to build a tunnel for boolean difference operations
		tunnel = this.gameObject.AddComponent<CircularTunnel> ();
	}


	public void BuildAnArch () {
		//doin preliminary calculations about the arc and scene clean-up
		PreliminaryCalc ();
		//centering the created arch (OddRow) about the origin in z direction
	...
	public void BuildARegularVault () {
		PreliminaryCalc ();
		//creating multiple OddRows one after another for regular vault
	...
	public void BuildLongZipVault () {
		PreliminaryCalc ();
		int row = 1;
		//when calculating vaultDepth/brickDepth, it rounds it to an integer, and that number is how many rows there 
		//should be in the vault. Once this number is known, odd and even rows of arch are drawn alternatingly
		while (row <= vaultDepth / brickDepth) {
			//starts with drawing odd row 
			OddRow().transform.position = new Vector3 (0, 0, -vaultDepth/2 + (row * brickDepth));
			row++;
			//if more rows need to be built, then draw even row
	...
	public void BuildTravZipVault () {
		PreliminaryCalc ();
		//Build first row. See Final ReadMe for details
		FirstRow ().transform.position = new Vector3 (0, 0, vaultDepth/2);
		//this variable is needed to keep track of the position of the last arch
		int j = 1;
		//build intermediate rows everywhere in between first and last rows
	...
	void PreliminaryCalc () { 
		//cleaning the scene of any existing circular arches (does not clean corbels)
		GameObject[] arches = GameObject.FindGameObjectsWithTag ("Arch");
		for (int i = 0; i < arches.Length; i++) {
			Destroy (arches [i].gameObject);
		}

		//destroys Tunnel associated with an arch (can also destroy corbel's tunnel, will destroy the tunnel which is on top of the Hierarchy)
		Destroy(GameObject.Find("Tunnel"));

		//throwing exeptions if there is invalid input for vaults only, so if arch is built, this piece of code is not executed
		StackTrace stackTrace = new StackTrace();
		if (stackTrace.GetFrame(1).GetMethod().Name != "BuildAnArch") {
			if (vaultDepth <= brickDepth) {
				//throws an error message and pauses the game in editor, but just stops executing the script in built game
				throw new Exception ("Vault depth cannot be less than brick depth. Check the input.");
			//alternatively, if the error and halting of the program is unwanted, the code can
			//ignore the given vault depth and builds it with brick depth, or ignore brick depth and build with vault depth
			}
		...
		//check if less than 2 parameters for the arch are provided
	...
	GameObject OddRow () {
		...
		for (int i = 1; i <= numberOfBricks; i++) {
			...
			//box colliders will be needed in boolean operations
			item.gameObject.AddComponent<BoxCollider>();
			//this part of the code takes u parameter for arc length, translates it into t parameter for arc, which is
			//then fed into wireArc.GetPoint to get position in world space for the point that u corresponds to. See Final ReadMe for details.
	...
	GameObject EvenRow () {
		...
		parent.transform.tag = "Arch";
		//create the first brick as half of the brick
		...
		//next brick's position will be (1/4 + 1/2) brick length away 
		...
		//last brick in even row is also half a brick
	...
	GameObject FirstRow () {
		...
		while (i <= numberOfBricks) {
			...
			u += innerBrickLength / curveLength;
			i++;
			//next brick is half depth. See Final ReadMe for details
			if (i <= numberOfBricks) {
	...
	GameObject IntermediateRow () {
		...
		int i = 1;
		while (i <= numberOfBricks) {
			//fist a full sized brick to fit into brick of the previous arch
			Transform item = Instantiate (prefab.transform) as Transform;
			item.transform.tag = "StructureBrick";
			item.gameObject.AddComponent<BoxCollider>();
			Vector3 position = wireArc.GetPoint (Map (u));
			...
			if (i <= numberOfBricks) {
				...
				item.gameObject.AddComponent<BoxCollider>();
				position = wireArc.GetPoint (Map (u));
				//shift the z position of this brick by half a brick, to fit into halfDepth brick of the previous row
	...
	GameObject LastRow () {
		...
		parent.transform.tag = "Arch";
		//skip the first brick, go to the second one
		float u = (3 * innerBrickLength/2)/curveLength;
		int i = 1;
		while (i < numberOfBricks) {
			//build only half depth bricks to fit into the previous arch
			...
			item.transform.parent = parent.transform;
			//move over 2 bricks, because only even bricks need half depth bricks to complete the vault
	...
	void BuildFullBrick () {
		//calculating the outer length of the brick; see Final ReadMe for details
		...
		//string (which defines the texture that should be used) "Brick" can be made into a parameter too 
		...
		//pass on dimensions for a full brick
		...
		//this piece of code is needed if you want the mesh to be attached to the prefab after you stop playing the game in the Editor;
		//will not work in a built game. See Final Readme for details
	...
	void BuildHalfBrick () {
		...
		//pass on dimensions of a half brick
		halfPrefab.GetComponent<MeshFilter>().mesh = CreateMesh (innerBrickLength/2, outerBrickLength/2, archThickness, brickDepth);
	...
	void BuildHalfDepthBrick () {
		...
		//pass on half a depth dimension
	...
	//See Final ReadMe for more details on how creating meshes work.
	...
	//arc-length parametrization. See Final ReadMe to see why this is needed
	float Map (float u) {
