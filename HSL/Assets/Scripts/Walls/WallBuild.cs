...
the constant numbers correspond to the cases when Positions script is enabled, ie when there is at ground at x,0,z*/
public class WallBuild : MonoBehaviour {
	...
	public void PopulateWallWithCreatedBrick () {
		...
		brickLength = this.transform.localScale.x;
		brickHeight = this.transform.localScale.y;
		brickDepth = this.transform.localScale.z;
	...
	void CreatePrefabBrick () {
		...
		brick.transform.localScale = this.transform.localScale;
	...
	void BrickCount () {
		brickCount.text = GameObject.FindGameObjectsWithTag ("Brick").Length.ToString ();
	}
}